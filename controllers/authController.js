...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //3)If everythong is ok,send token to client
  createSendToken(user._id, 200, res);
});

exports.protect = catchAsync(async (req, res, next) => {
  //1)Getting tooken and check of its there
  let token;
  if (
  ...
  if (!freshUser) {
    return next(
      new AppError('The token belogning to this user does not exist', 401),
    );
  }
  //4)CHeck if user changed password afer jwt  was issued
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  //3)Update changedPaswordAtt property for the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  //User.findbyidandupdate wll not work as intended!
